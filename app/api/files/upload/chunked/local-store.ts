...
export async function writeLocalChunkPart(input: {
  ...
  console.log("[chunked/local-store] Stored local chunk part", {
    uploadId,
    partId,
    partIndex,
    bytes: bytes.length,
...
export async function assembleLocalChunkedUpload(input: {
  ...
  for (const partId of partIds) {
    ...
    if (!trimmedPartId) {
      throw new Error("Local chunked upload completion payload included an empty part id.");
    }
    const bytes = await readFile(getPartPath(uploadId, trimmedPartId));

    // Copy into a plain ArrayBuffer so File constructor receives browser-compatible blob parts.
